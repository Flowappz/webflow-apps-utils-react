...
export const isKeyOf = <T extends string>(
  ...
  source: readonly T[]
): key is (typeof source)[number] => !!key && source.includes(key as T);
 ...
 * @returns
 */
export const getObjectKeys = <T extends Record<string, unknown>>(object: T): (keyof T)[] => Object.keys(object);
